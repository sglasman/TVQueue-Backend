...
import qualified Test.QuickCheck.Monadic as Q (run)
import App
...
createUserTest = quickCheck (\cur -> monadicIO $ do
  setup
...
loginTest = quickCheck (\email pass -> monadicIO $ do
  setup
  ...
  result3 <- run . handleLoginRequest $ LoginRequest email (pack $ unpack pass ++ "bad")
